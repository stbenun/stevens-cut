...
const PLANTS = [
  ...
  /* ---- the picker rewrite of 2026-09-08 — his instruction off another tracker: ticked foods ride
     ...
     staying green — and four of the six are about a NUMBER, which is the class that looks fine. */
  ...
  { guard: 'food-log', name: 'the ticked row prints its amount in the FACT’s unit while pricing his own',
    ...
    edits: [{ from: "    + '<span class=\"fv-s2\">' + p.n + ' ' + esc(p.u) + ' · ' + fvMacLine(p.m) + '</span>'",
              to:   "    + '<span class=\"fv-s2\">' + p.n + ' ' + esc(FOOD_FACTS[p.k].unit) + ' · ' + fvMacLine(p.m) + '</span>'" }] },
  ...
  { guard: 'food-log', name: 'the bulk add stops using the amount the row printed',
    ...
    edits: [{ from: "    if(logAddFood(ld, st.slot, p.k, p.n, p.u)) n++;",
              to:   "    if(logAddFood(ld, st.slot, p.k, 100, p.u)) n++;" }] },
  { guard: 'log-shape', name: 'the diary group header goes back to cal + protein, hiding the carbs and fat',
    edits: [{ from: "    const val = items.length ? Math.round(t[0]) + ' · ' + Math.round(t[1]) + 'P · '\n                               + Math.round(t[2]) + 'C · ' + Math.round(t[3]) + 'F'",
              to:   "    const val = items.length ? Math.round(t[0]) + ' · ' + Math.round(t[1]) + 'P'" }] },
  ...
  { guard: 'food-log', name: 'the ADD bar stops showing what the selection costs',
    edits: [{ from: "    + '<span class=\"fvadd-m\"><span class=\"fvadd-c\">' + Math.round(t[0]) + ' cal</span> · '\n    + Math.round(t[1]) + 'P · ' + Math.round(t[2]) + 'C · ' + Math.round(t[3]) + 'F</span>'",
              to:   "    + ''" }] },
...
,
  ...
  { guard: 'slot-budget-sum', name: "the recorded reason the slots do not sum to the target is deleted",
    ...
    edits: [{ from: "const SLOT_SUM_GAP = [-10, 4, 11, -1];",
              to:   "/* removed */" }] },
